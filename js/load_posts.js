...
async function postsLaden(postbyUserID = null) {
  ...
  if (posts.data.listPosts.counter == 0 && posts.data.listPosts.affectedRows.length == 0) {
    // no  post found
...
async function postClicked(objekt) {
  ...
  profile_header_left.addEventListener("click", function handledisLikeClick(event) {
    event.stopPropagation();
    event.preventDefault();
    redirectToProfile(objekt, this);
  });
