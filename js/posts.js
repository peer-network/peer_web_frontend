  ...
  return fetch(GraphGL, requestOptions)
    ...
    .then((result) => {
      ...
      if (result.data.resolvePostAction.status == "error") {
        throw new Error(
          userfriendlymsg(result.data.resolvePostAction.ResponseCode)
        );
      } else {
        return true;
      }
...
async function LiquiudityCheck(postCosts, title, action) {
  ...
  //console.log(action);
  if (action != "3") {
    // 3 means dislike and dislike is paid
  ...
  if (dailyPostAvailable) {
    let answer = await confirm(
      title,
      `🎉 This ${msg[action]} is free! You have ${freeActions[action]} free ${
        msg[action]
      }${freeActions[action] > 1 ? "s" : ""} available every 24 hours.`,
      (dontShowOption = true),
      msg[action]
    );
    if (answer === null || answer.button === cancel) {
    ...
    if (freeavailable === 0) {
      // if consumed free then reset popup for paid likes or comments or post
    ...
    let answer = await confirm(
      (title =
        '<div class="title-char"><img class="title-icon" src="svg/Exclude.svg" ><span>Insufficient Tokens</span></div>'),
      `<div class="modal-message">
        ...
        <div class="modal-message">
          <div>Like cost:</div> <div class="pricee"><span>${(
            postCosts * tokenPrice
          ).toFixed(
            2
          )}</span> <img src="svg/new_peerLogo.svg" alt="Peer Token" class="peer-token"></div>
        </div>`
      //(dontShowOption = true)
    );
    if (answer === null || answer.button === cancel) {
    ...
    let answer = await confirm(
      ...
      `<div class="modal-message">
        ...
        <div class="modal-message">
          <div>${msg[action]} cost:</div> <div class="pricee"><span>${(
        postCosts * tokenPrice
      ).toFixed(
        2
      )}</span> <img src="svg/new_peerLogo.svg" alt="Peer Token" class="peer-token"></div>
    ...
    await Merror(
      title,
      `You need ${(postCosts * tokenPrice).toFixed(2)} Peer Tokens to ${
        msg[action]
      }. Your balance is ${token} Peer Tokens.`
    ); //updated
    return false;
  }
